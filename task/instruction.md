...
## Context
`releasectl` is a small CI helper. Given a release bundle (manifest + eval report + smoke results), it prints SHIP/BLOCK and exits 0/1.

On-call has tickets that good builds with version `1.10.0` over `1.9.0` are blocked, accuracy improvements are called "regressions", and builds with a single failing smoke test still went out last week when the rest of the suite was green.

## Goal
Fix `releasectl` so release decisions match the policy below. Do not weaken the policy to make tests pass.
...
```bash
pip install -e .
releasectl decide --bundle fixtures/good_release --json
